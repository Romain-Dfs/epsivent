...
class DoctrineArtistRepository extends ServiceEntityRepository implements ArtistRepository
{
	...
	{
		$artistList = $this->findBy( [], [ 'id' => 'ASC' ] );

		return array_map( function ( ArtistEntity $artist ) {
			/** @var Artist $returnedArtist */
			$returnedArtist = new Artist( $artist->getName(), $artist->getDescription() );
			$returnedArtist->setId( $artist->getId() );
	...
	public function getArtist( int $artistId ): ?Artist
	{
		...
		return $artist ? new Artist( $artist->getName(), $artist->getDescription() ) : null;
	}
	...
	public function updateArtist( Artist $artist ): ?Artist
	{
		...
		if ( $updatedArtist ) {
			...
			return new Artist( $updatedArtist->getName(), $updatedArtist->getDescription() );
		}
